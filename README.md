# Walle-Mark.K

![OneBot12](https://img.shields.io/badge/OneBot-12-black?logo=data:image/png;base64,)
![version](https://img.shields.io/github/v/tag/onebot-walle/walle-k.svg)
![license](https://img.shields.io/github/license/onebot-walle/walle-k.svg)

[OneBot](https://onebot.dev/) 协议 [Kook](https://www.kookapp.cn/) 平台实现

## Event

- [x] message.private
- [x] message.channel
- [x] notice.reaction_decrease
- [ ] ...

## Action

- [x] send_message
- [ ] ...
